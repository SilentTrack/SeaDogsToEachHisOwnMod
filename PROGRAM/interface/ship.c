...
{
    InterfaceStack.SelectMenu_node = "LaunchShipState"; 
    ...
    EI_CreateFrame("SHIP_BIG_PICTURE_BORDER",156,78,366,313); 
	...
	switch (sCurrentNode)
	{
		...
		case "TABLE_OTHER":
		    ...
		    if (GameInterface.(CurTable).(CurRow).UserData.ID == "CannonType" && sti(xi_refCharacter.Ship.Cannons.Type) != CANNON_TYPE_NONECANNON)
		    {
		    	...
		    	sText2 = sText2 + NewStr() + "Recharge: "  + sti(GetCannonReloadTime(Cannon)) + " ���.";
		    	sText2 = sText2 + NewStr() + "Weight: "  + sti(Cannon.Weight) + " �.";
...
void ShowCannonsMenu()
{
	...
	bool bOk3 = bDisableMapEnter && !CheckAttribute(pchar, "GenQuest.MapClosedNoBattle") 
	if (bOk3 || bOk || bOk2)
    { 
...
void SetCannonsToBort(ref chr, string sBort, int iQty)
{
	...
	else
	{
		if (iQty < curQty)
		{
			SetCharacterGoods(chr, idx, GetCargoGoods(chr, idx) + (curQty - iQty)); 
		}
	}
