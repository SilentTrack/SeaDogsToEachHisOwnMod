...
void SetCamShuttle(ref loc) 
{
    if(CheckAttribute(loc, "camshuttle") == true)
	{
		if (CheckAttribute(pchar, "SystemInfo.ScaleSeaHeight"))
		{
			...
			Log_TestInfo("����� = "+pchar.SystemInfo.ScaleSeaHeight + "  ����� = " + sp);
			
			SendMessage(&LocCamera, "lffffff", -1, 0.5, -1.0, 1.0, 1.0, (0.07 * sp), (0.045 + sp/15.0)); 
		}
		else
		{
...
bool LoadLocation(ref loc)
{
	...
	if (isNoBoarding && loc.environment.sea == "true") LocLoadShips(loc); 
	...
	if(CheckAttribute(&mainCharacter, "lastFightMode") != 0)
	{
		...
		{
			if(CheckAttribute(loc, "noFight") && loc.noFight != "1") 
			{
	...
	{
		if (!IsLocationCaptured(loc.id) && !CheckAttribute(loc, "boarding"))
		{
	...
	{	
		...
		for (i=1; i<=10; i++)
		{
			CreateLocationParticles("large_smoke", "item", "smoke"+i, 1.0, 0, 0, ""); 
			CreateLocationParticles("large_smoke", "item", "smoke"+i, 1.0, 0, 0, ""); 
		}
		DoQuestFunctionDelay("Mtraxx_RetributionHellSplash", 1.5); 
	}
...
void LocLoadShips(ref Location)
{
	...
	if (bMainCharacterHere)
	{
		for (i=0;i<COMPANION_MAX;i++)
		{
			...
			locNumShips++;
			Characters[iCompanionIndex].Ship.Stopped = true; 
		}
	}
	...
	for (i=1;i<MAX_CHARACTERS;i++)  
	{
		...
		Characters[i].Ship.Stopped = true; 
	}
	...
	if (CheckAttribute(Location, "locators.reload.boat"))
	{
		if (bMainCharacterHere && iMainCharacterShipType != SHIP_NOTUSED)
		{
			if (CreateEntity(&locShips[n], "ship"))
			{
				...
				rCharacter = CharacterFromId("BoatChar"); 
				...
				rCharacter.Ship.stopped = true;
				rCharacter.Ship.Speed.z = 0.0;
				rCharacter.nation = sti(rPlayer.nation); 
...
{
	if (CheckAttribute(rCharacter,"location.from_sea") && !LAi_IsDead(rCharacter))  
	{
