...
int Group_CreateGroup(string sGroupID)
{
	...
	if (MOD_BETTATESTMODE == "On") Log_Info("Error: Group_CreateGroup ��� ����� � �������");
	return -1;
}
...
ref	Group_GetGroupByIndex(int iGroupIndex)
{
    if (iGroupIndex < 0) 
    {
        if (MOD_BETTATESTMODE == "On") Log_Info("Error: Group_GetGroupByIndex ��� ������");
...
ref	Group_FindOrCreateGroup(string sGroupID)
{
	...
	if (iGroupIndex < 0)
	{
		...
		if (iGroupIndex < 0) 
		{
			...
			if (MOD_BETTATESTMODE == "On") Log_Info("Error: Group_FindOrCreateGroup �� ��������� ������ " + sGroupID);
...
ref Group_GetGroupCommander(string sGroupID)
{
	...
	{
		trace("ERROR: Group_GetGroupCommander ��� ���������� ��� " + sGroupID);
		Log_Info("ERROR: Group_GetGroupCommander ��� ���������� ��� " + sGroupID);
	}
...
int Group_GetDeadCharactersNumR(ref rGroup)
{
	...
	for (int i=0; i<iNum; i++)
	{
		...
		if (sti(Characters[iCharacterIndex].Ship.Type) == SHIP_NOTUSED || LAi_IsDead(&Characters[iCharacterIndex])) { iDeads++; }  
	}
	return iDeads;
}
...
bool Group_isDeadR(ref rGroup)
{
	...
	while (true)
	{
		...
		if (sti(Characters[iCharacterIndex].Ship.Type) == SHIP_NOTUSED || LAi_IsDead(&Characters[iCharacterIndex])) { iNumDeadCharacters++; }  
		i++;
	}
...
float Group_GetPowerHP_R(ref rGroup)
{
	...
	while (true)
	{
		...
		if (iCharacterIndex < 0) { break; }
		if (CheckAttribute(&Characters[iCharacterIndex], "Ship.HP")) 
		{
...
void Group_CheckTask()
{
	...
	ref rGroup = Group_GetGroupByID(sGroupID);
    if (!CheckAttribute(rGroup, "Task")) 
    {
        if (MOD_BETTATESTMODE == "On") Log_Info("Error: Group_CheckTask ��� ������");
        return;
    }
