...
int AddCharacterGoodsCostCoeff(ref _refCharacter,int _Goods,int _Quantity, float costCoeff)
{
	...
	for (i=0; i<COMPANION_MAX; i++)
	{
		...
		if (cn!=-1 && GetRemovable(&Characters[cn]))//fix ������ ���� �����
		{
...
{
	int nShipType = GetCharacterShipType(_refCharacter);
	if( nShipType==SHIP_NOTUSED ) return 6; // -1 �����������, ����� ��������� ����, ��� ������ - ��� �����
...
{
	SetCrewQuantity(_refCharacter, GetMaxCrewQuantity(_refCharacter)); // ��������� � ��� �������� 01/08/06 boal
}
// <-- Eddy
...
void SetBaseShipData(ref refCharacter)
{
	...
	if (CheckAttribute(refCharacter,"Ship"))
	{
		...
		SetGoodsInitNull(refCharacter); // boal ������� � �����
		// ����� ����
...
{
	int curLocNum = FindLocation(_refCharacter.location);   // boal fix Characters[GetMainCharacterIndex()]    - ��-��
	...
	switch(groupID)
	{
		...
		case PATENT_ITEM_TYPE:
	        if(CheckAttribute(arItm,"quest"))
	        {
	            ...
	            chref.EquipedPatentId = itemNum; // boal ������� ������
	        }
	        else
	        {
		...
		case BLADE_ITEM_TYPE:
			...
			if(CheckAttribute(arItm,"Weight")) //eddy.��� �������� ������� ���� � �� ��� ������ - ������
			{
...
{
	if (CheckAttribute(chref, "GenQuest.ReputationNotChange")) return sti(chref.reputation); //eddy. ����� ����
...
int CheckItemInBox(string _itemID, string _locationID, string _box) // Addon 2016-1 Jason ������� ��������� ��������� � ���������� ������� ���������� �������
{
	int Qty = 0; 
    string attr;
...
// boal 08.01.05 -->
int GetPartyCrewQuantity(ref _refCharacter, bool _removable) // true - ���� ������ ����
{
	int cn;
	int sumCrew = 0;
	ref officer;
...
int GenerateNationTrade(int nation)
{
	...
	while (i == -1 && j < 10)
	{
		...
		if (ok || GetNationRelation(nation, i) == RELATION_ENEMY) // boal ���� nation == i || i == PIRATE- �� ���� ���� �����!!!
		{
			i = -1;
		}
		j++;
...
// Warship -->
// ������� �� ���������� ��� ������� ��������������
bool SetMainCharacterToMushketer(string sMushket, bool _ToMushketer) // ���� _ToMushketer == true, ������ ������ ����������, ����� - ������ �� ��������������
{
	int iItem;
	string sLastGun = "";
	...
	if(_ToMushketer && sMushket != "") // ������ �� ����������
	{
		...
		PChar.IsMushketer.LastGunID = sLastGun; // �������� ID ����������� �������
...
bool CanEquipMushketOnLocation(string LocationID)
{
	...
	/* if(HasSubStr(LocationID, "Tavern")) // � ������� ������
	{
		return false;
	} */
...
float ChangeIndianRelation(float _val) // Jason: ��������� � ��������
{
