void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Trader_distress":
			dialog.text = "Captain, we have had a talk already, right? You 'don't help profiteers', so what do you want from me now?";
		...
		case "EscortType_go":
			...
			npchar.lifeday = sti(pchar.GenQuest.Escort.Trader.DaysQty)+1; 
			...
			if (CheckAttribute(pchar, "GenQuest.Escort.Trader.Type3"))
			{
				...
				{
					if(sti(RealShips[sti(pchar.Ship.Type)].BaseType) <= sti(pchar.GenQuest.Escort.Trader.ShipType) && sti(RealShips[sti(pchar.Ship.Type)].BaseType) != SHIP_GALEON_H && GetCompanionQuantity(pchar) < 3)
					{
						pchar.quest.EscortTrader_Attack.win_condition.l1 = "location";
						pchar.quest.EscortTrader_Attack.win_condition.l1.location = pchar.GenQuest.Escort.Trader.Island;
		...
		case "EscortTrader_complete":
			...
			if (sti(sld.ship.HP) < makeint(sti(pchar.GenQuest.Escort.Trader.ShipMaxHP)/4))
			{
				dialog.text = "Captain, tell me why have I hired you? Look at my ship! What does she look like? She is a ruined! She is barely keeping above the water. Can't you see that? I don't get it - who was escorting who? Anyway, you won't get any reward, forget about it.";
			...
			if (sti(sld.ship.HP) < makeint(sti(pchar.GenQuest.Escort.Trader.ShipMaxHP)/2))
			{
		...
		case "Race_begin_2":
			dialog.text = "Well, not all of us are like that, friend. I always sail, I don't need any ballast. I perfectly know the Caribbean sea and might I say, I am both good in navigating, sailing and commanding my men. I am able to outrun any pirate.";
		...
		case "Race_begin_4":
			...
			if (sti(pchar.GenQuest.Racing.Go.DaysQty) < 1) pchar.GenQuest.Racing.Go.DaysQty = 1; 
		...
		case "Race_begin_5":
			dialog.text = "It is your call, captain.";
			iTemp = 1;
			for (i=5; i>=1; i--)
			{
					sTemp = "l"+iTemp;
		...
		case "Racing_rate5":
			...
			link.l1 = "This sum looks reasonable for me and I am not going to waste my time with your for the less. We are not beggars to bet for a few pesos. Fine... we have had a sit, enough now. If you fear to bet, It is your call... Boasting is cheap.";
		...
		case "Racing_rate_nomoney":
			...
			{
			dialog.text = "Well, you know we are not some beggars to bet for a few pesos or aren't you just scared? Fine, hell with you. But don't be so quick to judge other people, there are a lot of experienced sailors among merchants... much more experienced than you are.";
			link.l1 = "Fine, fine, don't sermonize me, philosopher. Farewell...";
			link.l1.go = "exit_sit";
			DeleteAttribute(pchar, "GenQuest.Racing.Go");
			NextDiag.TempNode = "First time";
		...
		case "Racing_Finished":
			...
			link.l1 = "Exactly, sir. But I want to tell you that you are surprisingly experienced and skilful for a captain of the trade ship. Not every merchant is able to sail that way on a flute for such a short time.";
			link.l1.go = "Racing_Finished_1";
			}
			else
			{
			dialog.text = "Well, aren't you surprised, captain? As I have said, I perfectly know the sea and sail my ship good enough. See now?";
		...
		case "Racing_Finished_1":
			dialog.text = "Well, you are the winner. Now you have to return to "+XI_ConvertString("Colony"+pchar.GenQuest.Racing.Go.StartCity)+" and take your money from the bartender. You have surely deserved them!";
		...
		case "Racing_end_repeat":
			dialog.text = "Is there is something else you want to talk about?";
